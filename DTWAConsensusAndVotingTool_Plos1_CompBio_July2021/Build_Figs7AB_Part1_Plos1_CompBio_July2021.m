function Build_Figs7AB_Part1_Plos1_CompBio_July2021
% function Build_Figs7AB_Part1_Plos1_CompBio_July2021
% M. Smith, Electrical and Computer Engineering, 
% University of Calgary, Calgary, Canada, July 2021
  ...
  applyZNORMBeforeDTWA = true;  % Worry about z-normalization in another paper
  
  for countStreams = 1 : length(numStreams) % Accept repeated answers above 128 with Sequin data 
    for countEnsemble = whichEnsemble : whichEnsemble % Hard coded for Enolase
      ...
      posnEnsembleEndStream = posnEnsembleStartStream + numStreams(countStreams) - 1;
      ...
      ensembleInfo.quietFig5Generation = true; % Obselete
...
%   Fig10_MakeLegends_Plos1_CompBio_July2021(votingLevels,  keepLegends, seperateFigs5, Gold2ConsensusFigure, MeanConsensus2Ensemble, SSGVotingFigure)
%   
% 
    ...
    for whichDTWA = 1 : length(DTWANames) % File name already has filled in %d_%d
      ...
      while ~feof(infileID)
        ...
        elseif contains(nextLine, '_goldToConsensus')
          if ~contains(nextLine, 'Normalized_Gold')
            ...
            if validValues ~= (numVotingLevels)
              [votingResults(whichEnsemble, whichDTWA).goldToConsensus, validValues] = sscanf(nextLine(7:end), '%f');
              if validValues ~= (numVotingLevels)
                fprintf('%d %d \n', validValues, numVotingLevels);
                 fclose(infileID);
              error('_goldToConsensus prolem with 6:end in 136 with large number -- e.g. 512 * 2 = 1024 -- get the 4 of 1024 -- try 7 : end');
              end
            end
          end
        ...
        elseif contains(nextLine, '_length_standard')
          ...
          if validValues ~= (numVotingLevels )
            [votingResults(whichEnsemble, whichDTWA).length_standard, validValues] = sscanf(nextLine(7:end), '%f');
            if validValues ~= (numVotingLevels )
              fclose(infileID);
              error('_length_standard  format ');
            end
          end
        elseif contains(nextLine, '_Consensus2Data_mean_dtwDistance')                
          if ~contains(nextLine, 'Normalized_Gold')
            ...
            if validValues ~= (numVotingLevels )
              [votingResults(whichEnsemble, whichDTWA).Consensus2Data_mean_dtwDistance, validValues] = sscanf(nextLine(7:end), '%f');
              if validValues ~= (numVotingLevels )
                fclose(infileID);
                error('_Consensus2Data_mean_dtwDistance format');
              end
            end
          end
        elseif contains(nextLine, '_Consensus2Data_std_dtwDistance')                     
          if ~contains(nextLine, 'Normalized_Gold')
            ...
            if validValues ~= (numVotingLevels)
              [votingResults(whichEnsemble, whichDTWA).Consensus2Data_std_dtwDistance, validValues] = sscanf(nextLine(7:end), '%f');
               if validValues ~= (numVotingLevels)
                 fclose(infileID);
                 error('_Consensus2Data_std_dtwDistance  format');
               end
            end
          end
        end
...
%% PLOS1_June2-21_Fig.6A 6B 6C -- OLD version  6A 6B new version
  if seperateFigs5
%% Obselete
%   MyPlotManySymbols_Plos1_CompBio_July2021(votingResults(whichEnsemble, MM).length_standard, votingResults(whichEnsemble, MM).goldToConsensus, '-k'); hold on; 
  else
    E_GL = 1308; 
